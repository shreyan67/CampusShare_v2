...
function PayoutsTab({ apiFetch }) {
  ...
  async function load() {
    ...
    try {
      ...
      const pending = sorted.filter(p => p.payoutStatus === "manual_pending").length
      const disputed = sorted.filter(p => p.payoutStatus === "disputed").length
      ...
      sessionStorage.setItem('prev_pending', pending.toString())
      sessionStorage.setItem('prev_disputed', disputed.toString())
